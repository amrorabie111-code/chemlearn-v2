...
export const ProfileScreen: React.FC<ProfileScreenProps> = ({
  ...
  const nextLevelXP = !levelInfo || levelInfo.name === 'Crystal' ? 9999 :
    levelInfo.name === 'Platinum' ? 1000 :
    levelInfo.name === 'Gold' ? 600 :
    levelInfo.name === 'Silver' ? 300 : 100;
  
  const progressToNext = !levelInfo || levelInfo.name === 'Crystal' ? 100 :
    ((user?.xp ?? 0) - levelInfo.minXP) / (nextLevelXP - levelInfo.minXP) * 100;
            ...
            <div className="text-center space-y-6">
              <div className="relative inline-block">
                <button
                  ...
                  className={`w-32 h-32 rounded-full overflow-hidden border-4 ${levelInfo?.borderColor} ${levelInfo?.glowColor || ''} transition-all duration-300 hover:scale-105`}
                >
                  {user.avatar ? (
                    <img 
                      src={user.avatar} 
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-center gap-2">
                  {getLevelIcon(levelInfo!.name as UserLevel)}
          ...
          <div className="flex justify-center py-2">
            <DayNightToggle 
              ...
              onToggle={() => {
                ...
                setTheme(newTheme);
                if (user) updateUserData({ theme: newTheme });
              }}
            />
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-4 space-y-4">
          ...
          <button 
            onClick={() => {
              ...
              setLanguage(newLang);
              if (user) updateUserData({ language: newLang });
            }}
